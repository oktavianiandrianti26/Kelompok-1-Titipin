...
const UlasanPengguna = () => {
  ...
  const handleReply = async (id) => {
    const balasan = document.getElementById(`balasan-${id}`).value;  // Ambil nilai balasan dari textarea
    ...
    try {
      ...
      // Setelah berhasil, ambil ulasan lagi untuk memperbarui tampilan
      fetchReviews();
    } catch (err) {
  ...
  async function handleSendNotification(payload) {
    console.log(payload);
    const response = await axios.post(`http://localhost:3000/api/user/notifications/push-notifications`, payload, {
      headers: {
        "Content-Type": "application/json",
      }
    });
    alert("Notifikasi berhasil dikirim");  // Menampilkan pesan dari server
        ...
        <div className="p-4 sm:p-6 flex-1">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {reviews.map((review) => (
              ...
              >
                <div className="mb-2">
                  <p className="font-medium text-gray-700">{review.user_id?.nama || "N/A"}</p>
                  <p className="text-sm text-gray-500">{review.user_id?.email || "N/A"}</p>
                </div>
                ...
                <textarea
                  id={`balasan-${review._id}`}  // Set ID unik untuk textarea
